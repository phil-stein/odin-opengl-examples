...
input_init :: proc()
{
  glfw.SetKeyCallback( window, glfw.KeyProc(input_key_callback) )
  glfw.SetCursorPosCallback( window, glfw.CursorPosProc(input_mouse_pos_callback) )
}
...
{
  mouse_delta_x = f32(xpos) - mouse_x
  mouse_delta_y = mouse_y - f32(ypos) // for some reason y is invers, prob because opengl is weird about coordinates
...
{
  glfw.SetCursorPos( window, f64(window_width / 2), f64(window_height / 2) )
  mouse_x = f32(window_width / 2)
  mouse_y = f32(window_height / 2)
  mouse_delta_x = 0 
  mouse_delta_y = 0
}
...
{
  glfw.SetInputMode( window, glfw.CURSOR, visible ? glfw.CURSOR_NORMAL : glfw.CURSOR_DISABLED )
}
