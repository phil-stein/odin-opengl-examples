...
uniform mat4 inv_model;     // matrix for applying the objects transform
...
void main() 
{
  ...
  _out.normal     = mat3(transpose(inv_model)) * aNormal;
  ...
  // gl_Position = proj * vec4(aPosition.xyz, 1.0);
  // gl_Position = model * vec4(aPosition.xyz, 1.0);
  // gl_Position = proj * model * vec4(aPosition.xyz, 1.0);
  // gl_Position = view * vec4(aPosition.xyz, 1.0);
}
